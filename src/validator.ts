...
import {
  validateCrossLayerRefTargets,
  validateStructuredReferencesKind,
  validateLegacyRefFieldMigration,
} from './rules/v17/index.js';
...
export async function validateBrand(
  ...
  const visitedFiles: { rel: string; data: Record<string, unknown> | null }[] = [];
...
async function validateNamedFile(
  layerName: string,
  entry: FileEntry,
  relPath: string,
  brandPath: string,
...
async function parseAndValidateFile(
  ...
  try {
    ...
    collector.error('file-read', relPath, `could not read file: ${(err as Error).message}`);
    return;
  }
  ...
  if (entry.frontmatter && Object.keys(entry.frontmatter).length > 0) {
    validateFrontmatterFields(
      data,
      entry.frontmatter as FrontmatterMap,
      relPath,
      `layer:${layerName}:frontmatter`,
      collector,
    );
  }
    ...
    if (entry.path) {
      ...
      if (entry.path.endsWith('/')) {
        if (entry.required && !(await isDir(absPath))) {
          collector.error(
            `layer:${layerName}:required-subdir`,
            relPath,
            `required subdirectory is missing`,
          );
        }
        continue;
      }
      ...
      for (const f of files) {
        if (f.startsWith('_')) continue; // reserved-prefix convention: skip framework/archive
        if (fnameRe.test(f)) {
      ...
      if (required && matches.length < min) {
        ...
        collector.error(
          `layer:${layerName}:min-files`,
          dirRel + '/',
          `pattern '${fullPattern}' requires at least ${min} match(es); found ${matches.length}`,
        );
      }
      ...
      if (entry.required && matches.length < min) {
        collector.error(
          `layer:${layerName}:glob-min`,
          dirRel + '/',
          `glob '${entry.glob}' requires at least ${min} match(es); found ${matches.length}`,
        );
      }
        ...
        for (const f of matches) {
          ...
          try {
            ...
            collector.error(
              'data-files-yaml',
              relPath,
              `not valid YAML: ${(err as Error).message}`,
            );
          }
        }
      }
...
async function runCrossLayerRules(
  brandPath: string,
  spec: BrandSpec,
  collector: IssueCollector,
  visited: VisitedFile[],
  ...
  await ruleLayerFolderIndex(
    brandPath,
    'identity',
    'identity-folder-index',
    collector,
  );
  await ruleLayerFolderIndex(
    brandPath,
    'vocabulary',
    'vocabulary-folder-index',
    collector,
  );
  await ruleLayerFolderIndex(
    brandPath,
 ...
 */
async function ruleLayerPatternExclusive(
  brandPath: string,
  fileName: string,
  dirName: string,
  ...
  if (layerRequired && !hasFile && !hasDir) {
    collector.error(
      requiredRuleId,
      '',
      `${dirName} layer missing: provide either ${fileName} OR a ${dirName}/ directory with a canonical entry point (README.md or _framework.md)`,
    );
    return;
  }
  if (hasFile && hasDir) {
    collector.error(
      exclusiveRuleId,
      '',
      `both ${fileName} and ${dirName}/ directory present; brand must use exactly one ${dirName} pattern`,
    );
  }
}
 ...
 */
async function ruleLayerFolderIndex(
  brandPath: string,
  dirName: string,
  ruleId: string,
  collector: IssueCollector,
  ...
  if (!hasReadme && !hasFramework) {
    collector.error(
      ruleId,
      `${dirName}/`,
      `${dirName}/ directory must contain a canonical entry point (README.md or _framework.md)`,
    );
  }
  if (hasReadme && hasFramework) {
    collector.error(
      ruleId,
      `${dirName}/`,
      `${dirName}/ must contain exactly one of README.md or _framework.md, not both`,
    );
  }
}

async function ruleVoiceExclusive(brandPath: string, collector: IssueCollector): Promise<void> {
  ...
  if (!hasFile && !hasDir) {
    collector.error(
      'voice-required',
      '',
      `voice layer missing: provide either voice.md OR a voice/ directory with voice/registers/{slug}.md files`,
    );
    return;
  }
  if (hasFile && hasDir) {
    collector.error(
      'voice-pattern-exclusive',
      '',
      `both voice.md and voice/ directory present; brand must use exactly one voice pattern`,
    );
  }
}

async function ruleVoiceMultiRegisterIndex(
  brandPath: string,
  collector: IssueCollector,
): Promise<void> {
  const voiceDir = path.join(brandPath, 'voice');
  if (!(await isDir(voiceDir))) return;
  ...
  if (!hasReadme && !hasIndex) {
    collector.error(
      ...
      'voice/',
      `multi-register voice/ directory must contain README.md or _index.md`,
    );
  }
  if (hasReadme && hasIndex) {
    collector.error(
      'voice-multi-register-index',
      'voice/',
      `voice/ must contain exactly one of README.md or _index.md, not both`,
    );
  }
  ...
  if (!(await isDir(registersDir))) {
    collector.error(
      'voice-multi-register-index',
      'voice/registers/',
      `voice/registers/ directory required when using multi-register pattern`,
    );
    return;
  }
  ...
  if (registerFiles.length < 1) {
    collector.error(
      'voice-multi-register-index',
      'voice/registers/',
      `at least one register file required under voice/registers/`,
    );
  }
    ...
    try {
      ...
      if (!data || typeof data['register'] !== 'string') {
        collector.error(
          'voice-multi-register-index',
          rel,
          `register file must have frontmatter.register populated`,
        );
      }
      ...
      if (!Array.isArray(appliesTo) || appliesTo.length === 0) {
        collector.error(
          'voice-multi-register-index',
          rel,
          `register file must have frontmatter.applies_to_refs (or legacy applies_to) populated (non-empty array)`,
        );
      }
    } catch (err) {
      if (err instanceof FrontmatterError) {
...
async function ruleAgentContextPriorityLayers(
  brandPath: string,
  spec: BrandSpec,
  collector: IssueCollector,
): Promise<void> {
  ...
  try {
    ...
    for (const l of layers) {
      ...
      if (!validLayers.has(l)) {
        collector.error(
          'agent-context-priority-layers-resolve',
          relPath,
          `priority_layers entry '${l}' is not a declared layer (valid: ${[...validLayers].join(', ')})`,
        );
      }
    }
  } catch {
...
async function rulePromptValidatedExamples(
  brandPath: string,
  collector: IssueCollector,
): Promise<void> {
  ...
  for (const f of files) {
    ...
    if (!data || data['validated'] !== true) continue;
    const examples = data['examples'];
    if (!Array.isArray(examples) || examples.length === 0) {
      collector.error(
        'prompt-example-when-validated',
        rel,
        `validated: true requires non-empty examples array in frontmatter`,
      );
      continue;
    }
    for (const e of examples) {
      ...
      if (!(await exists(candidate))) {
        collector.error(
          'prompt-example-when-validated',
          rel,
          `examples entry '${e}' does not exist in the brand repo`,
        );
      }
    }
  }
...
async function ruleUiTokensLayerOrder(brandPath: string, collector: IssueCollector): Promise<void> {
  ...
  if (hasSem && !hasPrim) {
    collector.error(
      'ui-tokens-layer-order',
      'ui-tokens/semantic.md',
      `ui-tokens/semantic.md exists but ui-tokens/primitives.md is missing`,
    );
  }
  if (hasComp && !hasSem) {
    collector.error(
      'ui-tokens-layer-order',
      'ui-tokens/component.md',
      `ui-tokens/component.md exists but ui-tokens/semantic.md is missing`,
    );
  }
}
...
async function ruleUiTokensDependsOn(brandPath: string, collector: IssueCollector): Promise<void> {
  ...
  for (const f of files) {
    ...
    const depsRaw = (data['depends-on'] ?? data['depends_on']) as unknown;
    if (!Array.isArray(depsRaw)) continue;
    for (const dep of depsRaw) {
      ...
      if (!present.has(base)) {
        collector.error(
          'ui-tokens-depends-on-resolves',
          rel,
          `depends-on entry '${dep}' does not resolve to a sibling under ui-tokens/`,
        );
      }
    }
  }
...
async function ruleAllFrontmatterParses(
  brandPath: string,
  collector: IssueCollector,
  visited: VisitedFile[],
): Promise<void> {
  ...
  for (const rel of all) {
    ...
    if (visitedSet.has(rel)) continue;
...
async function ruleFreshness(
  ...
  for (const f of files) {
    ...
    if (!data) continue;
    const lastUpdated =
      (data['last_updated'] as string | undefined) ?? (data['updated'] as string | undefined);
    if (typeof lastUpdated !== 'string') continue;
    ...
    if (days > thresholdDays) {
      collector.warn(
        ruleId,
        rel,
        `last_updated ${lastUpdated} is ${days} days old (threshold: ${thresholdDays})`,
      );
    }
  }
}
