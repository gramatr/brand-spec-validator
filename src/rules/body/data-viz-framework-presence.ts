...
const EXPECTED_TOPICS: { label: string; matchers: RegExp[] }[] = [
...
export async function validateDataVizFrameworkPresence(
  brandPath: string,
  cache: BodyCache,
  collector: IssueCollector,
): Promise<void> {
  const fwPath = path.join(brandPath, 'data-viz', '_framework.md');
  if (!(await isFile(fwPath))) return; // companion rule already warns on absence
  ...
  for (const topic of EXPECTED_TOPICS) {
    ...
    if (!covered) {
      collector.info(
        'data-viz-framework-presence',
        'data-viz/_framework.md',
        `framework body should cover '${topic.label}' (no heading matched expected topic patterns)`,
      );
    }
  }
}
