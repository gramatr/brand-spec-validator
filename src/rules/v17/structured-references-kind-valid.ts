...
const VALID_KINDS = ['requires', 'recommends', 'cites'] as const;
const VALID_TYPES = [
  'layer',
  'file',
  'token',
...
function checkEntry(
  ...
  for (const required of ['type', 'target', 'kind']) {
    if (!Object.prototype.hasOwnProperty.call(entry, required)) {
      collector.error(
        RULE_ID,
        filePath,
        `references.${entryKey} missing required field '${required}'`,
      );
    }
  }

  const kind = entry['kind'];
  if (kind !== undefined && !VALID_KINDS.includes(kind as never)) {
    collector.error(
      RULE_ID,
      filePath,
      `references.${entryKey}.kind = ${JSON.stringify(kind)} is not in [${VALID_KINDS.join(', ')}]`,
    );
  }

  const type = entry['type'];
  if (type !== undefined && !VALID_TYPES.includes(type as never)) {
    collector.error(
      RULE_ID,
      filePath,
      `references.${entryKey}.type = ${JSON.stringify(type)} is not in [${VALID_TYPES.join(', ')}]`,
    );
  }
  ...
  if (Object.prototype.hasOwnProperty.call(entry, 'entries') && type !== 'layer-entries') {
    collector.warn(
      RULE_ID,
      filePath,
      `references.${entryKey}.entries is only meaningful when type: layer-entries (got type: ${JSON.stringify(type)})`,
    );
  }
}

export async function validateStructuredReferencesKind(
  brandPath: string,
  collector: IssueCollector,
): Promise<void> {
  ...
  for (const rel of all) {
    ...
    if (!isObject(refs)) continue;
    for (const [key, entry] of Object.entries(refs)) {
      if (!isObject(entry)) {
        collector.error(
          RULE_ID,
          rel,
          `references.${key} must be a mapping (object) with type/target/kind fields`,
        );
        continue;
      }
