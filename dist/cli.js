...
program
    ...
    .option('--spec <path>', 'override path to a checkout of the gramatr/brand-spec repo (defaults to the vendored copy that ships with this package)')
    .option('--format <format>', 'output format: human | json', 'human')
    .option('--quiet', 'suppress warnings; show errors only', false)
    .option('--no-freshness', 'skip freshness warning rules')
    .action(async (brandPath, opts) => {
    try {
