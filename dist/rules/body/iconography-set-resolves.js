...
function isCustomIndicator(value) {
    const v = value.toLowerCase().trim();
    if (PUBLIC_LIBRARIES.has(v))
        return false;
...
export async function validateIconographySetResolves(brandPath, cache, collector) {
    const iconographyMd = path.join(brandPath, 'design', 'iconography.md');
    if (!(await isFile(iconographyMd)))
        return;
    let setName;
    try {
    ...
    if (!isCustomIndicator(setName)) {
        ...
        collector.info('iconography-set-resolves', 'design/iconography.md', `icon_set '${setName}' resolves to a known public library; local design/icons/ files are not required`);
        return;
    }
    ...
    if (!(await isDir(iconsDir))) {
        collector.info('iconography-set-resolves', 'design/icons/', `icon_set '${setName}' indicates a brand-provided set; design/icons/ directory is missing`);
        return;
    }
    const files = await listFiles(iconsDir);
    const iconFiles = files.filter((f) => ICON_EXTS.some((ext) => f.toLowerCase().endsWith(ext)));
    if (iconFiles.length === 0) {
        collector.info('iconography-set-resolves', 'design/icons/', `icon_set '${setName}' indicates a brand-provided set; design/icons/ contains no icon source files (looked for ${ICON_EXTS.join(', ')})`);
    }
    if (!files.includes('_manifest.md')) {
        collector.info('iconography-set-resolves', 'design/icons/', `icon_set '${setName}' indicates a brand-provided set; design/icons/_manifest.md describing the set is missing`);
    }
}
//# sourceMappingURL=iconography-set-resolves.js.map