...
function looksLikeAllowedSection(headingPath) {
    ...
    for (const h of headingPath) {
        if (NEGATIVE_HEADING_PATTERNS.some((re) => re.test(h)))
            return false;
        if (ALLOWED_HEADING_PATTERNS.some((re) => re.test(h)))
            permissive = true;
    }
    return permissive;
}
async function readForbiddenTerms(brandPath, cache) {
    const vocabPath = path.join(brandPath, 'vocabulary.md');
    if (!(await isFile(vocabPath)))
        return [];
    ...
    for (const sec of sections) {
        ...
        while ((m = inlineRe.exec(sec.text)) !== null) {
            ...
            if (candidate && /^[A-Za-z][\w' -]*$/.test(candidate))
                terms.add(candidate);
        }
    }
    return [...terms];
}
export async function validateRegisterInheritance(brandPath, cache, collector) {
    ...
    if (forbidden.length === 0)
        return;
    const files = (await listFiles(registersDir)).filter((f) => f.endsWith('.md') && !f.startsWith('_'));
    for (const f of files) {
        ...
        for (const sec of body.sections) {
            if (!looksLikeAllowedSection(sec.headingPath))
                continue;
            for (const term of forbidden) {
                const re = new RegExp(`\\b${escapeRegExp(term)}\\b`, 'i');
                if (re.test(sec.text)) {
                    collector.error('register-inheritance-cannot-subtract', rel, `forbidden term '${term}' appears under an allowed/permitted section ('${sec.heading}'); registers MUST NOT re-allow brand-wide forbidden terms`);
                }
            }
        }
    }
...
//# sourceMappingURL=register-inheritance.js.map