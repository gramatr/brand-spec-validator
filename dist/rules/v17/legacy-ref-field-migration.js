 ...
 */
function isNestedSourceAuthority(context) {
    return context === 'source_authority';
}
export async function validateLegacyRefFieldMigration(brandPath, collector) {
    ...
    for (const rel of all) {
        ...
        for (const alias of FIELD_ALIASES) {
            if (isNestedSourceAuthority(alias.context)) {
                ...
                if (Object.prototype.hasOwnProperty.call(sa, alias.canonical))
                    continue;
                for (const legacy of alias.legacy) {
                    if (Object.prototype.hasOwnProperty.call(sa, legacy)) {
                        collector.info(RULE_ID, rel, `field 'source_authority.${legacy}' is the legacy name; consider migrating to 'source_authority.${alias.canonical}' per the v1.7 cross-layer-ref convention`);
                        break;
                    }
                }
            }
                ...
                for (const legacy of alias.legacy) {
                    if (Object.prototype.hasOwnProperty.call(data, legacy)) {
                        collector.info(RULE_ID, rel, `field '${legacy}' is the legacy name; consider migrating to '${alias.canonical}' per the v1.7 cross-layer-ref convention`);
                        break;
                    }
                }
            }
...
//# sourceMappingURL=legacy-ref-field-migration.js.map