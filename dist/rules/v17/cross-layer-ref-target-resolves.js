...
function isTokenShape(s) {
    return TOKEN_SHAPE_RE.test(s);
}
function isPathWithFragment(s) {
    const m = s.match(PATH_FRAGMENT_RE);
    if (!m || !m[1] || !m[2])
        return null;
    return { file: m[1], anchor: m[2] };
}
async function resolveOne(value, fieldName, filePath, brandPath, layerNames, collector) {
    if (typeof value !== 'string' || value.trim() === '') {
        collector.error(RULE_ID, filePath, `cross-layer reference '${fieldName}' has non-string or empty value: ${JSON.stringify(value)}`);
        return;
    }
    const v = value.trim();
    ...
    if (isTokenShape(v)) {
        collector.warn(RULE_ID, filePath, `cross-layer reference '${fieldName}' value '${v}' is token-shaped; full token resolution depends on the body-parse subsystem and is deferred (warn-only per v1.7 spec)`);
        return;
    }
    ...
    if (pf) {
        ...
        if (!(await exists(candidate))) {
            collector.error(RULE_ID, filePath, `cross-layer reference '${fieldName}' file '${pf.file}' does not exist (anchor verification deferred to body-parse)`);
        }
        return;
    }
    ...
    if (!(await exists(candidate))) {
        ...
        if (!v.includes('/') && !v.includes('.')) {
            collector.error(RULE_ID, filePath, `cross-layer reference '${fieldName}' value '${v}' is not a declared layer and does not exist as a file in the brand repo`);
        }
        else {
            collector.error(RULE_ID, filePath, `cross-layer reference '${fieldName}' file '${v}' does not exist in the brand repo`);
        }
    }
}
export async function validateCrossLayerRefTargets(brandPath, spec, collector) {
    ...
    for (const rel of all) {
        ...
        for (const [name, raw] of Object.entries(data)) {
            if (!REF_FIELD_RE.test(name))
                continue;
            if (NON_RESOLVING_REF_FIELDS.has(name))
                continue;
            ...
            if (isPlural) {
                if (!Array.isArray(raw)) {
                    ...
                    collector.error(RULE_ID, rel, `cross-layer reference '${name}' must be an array (plural form); got ${typeof raw}`);
                    continue;
                }
                ...
                // singular *_ref
                if (Array.isArray(raw)) {
                    collector.error(RULE_ID, rel, `cross-layer reference '${name}' must be a scalar (singular form); got array`);
                    continue;
                }
                await resolveOne(raw, name, rel, brandPath, layerNames, collector);
            }
...
//# sourceMappingURL=cross-layer-ref-target-resolves.js.map